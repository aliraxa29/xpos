...
def collect_stock_errors(items, pos_profile=None):
...
def validate_stock_on_invoice(invoice_doc):
	...
	errors = collect_stock_errors(items_to_check, pos_profile=pos_profile)
...
class XPosInsufficientStockError(frappe.ValidationError):
	...
	A distinct class so clients can recognise the refusal without matching on
	the (translated) message - the offline queue uses it to mark an invoice
	terminally rejected instead of retrying it forever.  Frappe echoes the
	class name back as ``exc_type``.
	"""
